...
function NewBook(){
    ...
    useEffect(() => {
      ...
      if (location.state?.book) {
        setBook(location.state.book);  // Atualiza o estado com o livro passado
      } else if (id) {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        if (id) {
          await axios.put(`http://localhost:8080/v1/book/${id}`, book);
        } else {
          await axios.post("http://localhost:8080/v1/book", book);
        }
        navigate("/book-list");
      } catch (error) {
      ...
      <div className="container mt-5">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            ...
            <select className="form-select" name="authors" value={book.authors} onChange={handleChange} required>
